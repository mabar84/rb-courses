...
const path = {
  ...
  source: {
    ...
    vendor: ['src/vendor/**/*', '!src/vendor/_libs', '!src/vendor/_libs/**/*', '!src/vendor/**/node_modules', '!src/vendor/**/node_modules/**/*']
  },
  watch: {
    html: ['src/**/*.html'],
    js: ['src/js/**/*.js'],
...
function vendors(cb) {
  gulp.src(path.source.vendor, {dot: true}).pipe(gulp.dest(path.build.vendor));
  cb();
}
...
function font2woff(cb) {
  gulp.src(path.source.fontsConvert).pipe(ttf2woff()).pipe(gulp.dest(path.build.fonts));
  cb();
}

function font2woff2(cb) {
  gulp.src(path.source.fontsConvert).pipe(ttf2woff2()).pipe(gulp.dest(path.build.fonts));
  cb();
}

function srcOTF2srcTTF(cb) {
  gulp.src(path.source.fontsOTF).pipe(fonter({ formats: ['ttf'] })).pipe(gulp.dest('src/fonts/'));
  cb();
}
...
const build = gulp.parallel(html, js, styles, stylesCSS, vendors, c_font, copyImages);
